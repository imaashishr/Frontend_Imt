...
import InvoicesTable from "../invoice/InvoiceTable.jsx"; // Import Invoice Table
    ...
    <div className="flex">
      ...
      <div className="flex-1 p-8 bg-gray-50 min-h-screen relative">
        ...
        <h1 className="text-4xl font-bold text-center text-[#3B71CA] mb-5">
          {user1.role === "vendor" ? "Vendor Details" : user1.role === "admin" ? "Vendors List" : "EI Information"}
        </h1>

        {user1.role === "vendor" && (
          <div className="bg-white rounded-lg shadow-xl overflow-hidden p-8 w-full max-w-5xl mx-auto">
            <h2 className="text-3xl font-semibold text-black-600 mb-4">Vendor Id: {user.lifnr}</h2>
            <h2 className="text-3xl font-semibold text-blue-600 mb-4">Vendor Name: {user.name}</h2>
          </div>
        )}

        {/* Display Invoice Table for Vendor */}
          ...
          <div className="space-y-8 flex flex-col items-center">
            {data.map((vendor, index) => (
              <div key={index} className="bg-white rounded-lg shadow-xl overflow-hidden p-8 w-full max-w-5xl">
                <h2 className="text-3xl font-semibold text-black-600 mb-4"><strong>Vendor id:</strong> {vendor.Lifnr}</h2>
                <h2 className="text-3xl font-semibold text-blue-600 mb-4"><strong>Vendor Name:</strong> {vendor.Name1}</h2>
              </div>
            ))}
          </div>
        ) : user1.role === "admin" ? (
        ...
        {user1.role === "Ei" && (
          <div className="bg-white rounded-lg shadow-xl overflow-hidden p-8 w-full max-w-5xl mx-auto">
            <h2 className="text-3xl font-semibold text-blue-600 mb-4">EI Name: {user.name}</h2>
            <p className="font-medium text-gray-700"><strong>EI id:</strong> {user.id}</p>
